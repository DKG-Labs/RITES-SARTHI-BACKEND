...
/**
 * Parent Entity for Final Inspection - Weight Test
 *
...
@Table(name = "final_weight_test", indexes = {
    ...
    @Index(name = "idx_final_weight_unique", columnList = "inspection_call_no,lot_no,heat_no", unique = true)
})
@Data
public class FinalWeightTest {
     ...
     * One-to-many relationship with FinalWeightTestSample.
     */
    @OneToMany(mappedBy = "finalWeightTest", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.LAZY)
    private List<FinalWeightTestSample> samples;
