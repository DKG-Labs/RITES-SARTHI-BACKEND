...
/**
 * Child Entity for Final Inspection - Hardness Test Sample
 * 
 ...
 * - Overall status is stored ONLY in final_hardness_test.status.
 */
@Entity
@Table(name = "final_hardness_test_sample", indexes = {
    @Index(name = "idx_fhts_parent_id", columnList = "final_hardness_test_id"),
    @Index(name = "idx_fhts_sampling_no", columnList = "sampling_no"),
    @Index(name = "idx_fhts_unique", columnList = "final_hardness_test_id,sampling_no,sample_no", unique = true)
})
@Data
public class FinalHardnessTestSample {
    ...
    /**
     * Foreign key to final_hardness_test.
     * Identifies which inspection session this sample belongs to.
     */
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "final_hardness_test_id", nullable = false)
    private FinalHardnessTest finalHardnessTest;
    ...
    /**
     * Actual hardness value entered by the inspector.
     */
    @Column(name = "sample_value", nullable = false, precision = 10, scale = 6)
