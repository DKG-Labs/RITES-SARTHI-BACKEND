...
/**
 * Parent Entity for Final Inspection - Freedom from Defects Test
 *
 ...
 */
@Entity
@Table(name = "final_freedom_from_defects_test", indexes = {
    @Index(name = "idx_final_freedom_call_no", columnList = "inspection_call_no"),
    @Index(name = "idx_final_freedom_lot_no", columnList = "lot_no"),
    @Index(name = "idx_final_freedom_heat_no", columnList = "heat_no")
})
@Data
public class FinalFreedomFromDefectsTest {
     ...
     */
    @OneToMany(mappedBy = "finalFreedomFromDefectsTest", cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.LAZY)
    private List<FinalFreedomFromDefectsSample> samples;
